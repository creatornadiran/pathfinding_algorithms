...
class Node:
    ...
    def __lt__(self,other):
        return False
def h(p1,p2): #heuristic distance (Manhattan dist.)
    x1,y1 = p1
    x2,y2 = p2
...
def algorithm(draw, grid, start, end):
    count =0 #insertion order
    open_set = PriorityQueue()
    open_set.put((0, count, start)) #Sorts by 1. g_score 2. count
    start.g_score = 0
    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            ...
            if temp_g_score < neighbor.g_score: #If algorithm finds a more efficient way updates g score with efficient one and opens it
